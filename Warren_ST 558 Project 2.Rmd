...
securityType <- function(vec = NULL){
  ...
  # Get text set up
  jsonString <- "https://api.fiscaldata.treasury.gov/services/api/fiscal_service/v2/accounting/od/avg_interest_rates?limit=4500"
  
  addedText <- if (is.null(vec) == TRUE) "" else if(all(vec %in% testingList) == TRUE) paste("&filter=security_type_desc:in:(", paste(URLencode(vec), sep = "", collapse = ","), ")", sep = '') else stop("Please input a value that is acceptable. You can find the acceptable values by using the helper function getSecurityList(). Also make sure inputs are in quotations and spelled in the same way you can find in the helper function.")
...
individualSecurities <- function(vec = NULL){
  ...
  addedText <- if (is.null(vec) == TRUE) "" else if(all(vec %in% testingList) == TRUE) paste("&filter=security_desc:in:(", paste(URLencode(vec), sep = "", collapse = ","), ")", sep = '') else stop("Please input a value that is acceptable. You can find the acceptable values by using the helper function getSecurityList('specific securities'). Also make sure inputs are in quotations and spelled in the same way you can find in the helper function.")
...
filterDate <- function(date1 = NULL, date2 = NULL, operator = NULL){
  # Get text set up
  jsonString <- "https://api.fiscaldata.treasury.gov/services/api/fiscal_service/v2/accounting/od/avg_interest_rates?limit=4500"
  
  addedText <- if ((is.null(operator) == TRUE) | ((is.null(date1) == TRUE) & is.null(date2) == TRUE)) "" else if(operator %in% c("lt", "gt", "lte", "gte", "eq")) paste("&filter=record_date:", operator, ":", URLencode(date1), sep = "") else if (operator %in% "in") paste("&filter=record_date:gte:", URLencode(date1), ",record_date:lte", URLencode(date2), sep = "") else stop("Please input a value that is acceptable. Both date options must be in 'yyyy-mm-dd' format and the starting date for the data is '2001-01-31'. The inputs for the operator argument are either 'lt' for less than, 'lte' for less than or equal to, 'gt' for greater than, 'gte' for greater than or equal to, 'eq' for equal to, and 'in' if you want between two dates. Also make sure inputs are in quotations and spelled in the same way you can find in this message.")
